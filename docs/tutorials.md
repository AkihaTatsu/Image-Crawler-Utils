...
+ `timeout`: Timeout (seconds) for waiting elements. If no element is loaded within `timeout` seconds, an error is logged and the function will return `None`. Default is 30.
...
+ `waiting_seconds`: In the case of `headless=True`, if failed to log in within `waiting_seconds` seconds, an error is logged and the function will return `None`. Default is 60.
